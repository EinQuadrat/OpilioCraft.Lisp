...
let funcIsValidObjectPath (contextProvider: ObjectPathContextProvider) (_: Environment) (exprList: Expression list) : Expression =
    match exprList with
...
let funcLookupObjectPath (contextProvider: ObjectPathContextProvider) (_: Environment) (exprList: Expression list) : Expression =
    match exprList with
...
// supportive macros
let macroPropertyIs _ (exprList: Expression list) : Expression =
    match exprList with
...
let macroPropertyIsNot _ (exprList: Expression list) : Expression =
    match exprList with
    | [ Atom (FlexibleValue.String _); Atom _ ] as [ objectPath ; pattern ] ->
...
let macroPropertyContains _ (exprList: Expression list) : Expression =
    match exprList with
...
let macroPropertyMatches _ (exprList: Expression list) : Expression =
    match exprList with
    | [ Atom _ ; Atom _ ] as [ objectPath ; pattern ] ->
